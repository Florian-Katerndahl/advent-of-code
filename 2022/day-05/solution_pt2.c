...
void execute_instruction(struct Stack *stack, struct Instructions *instructions) {
    char stack_of_crates[STACK_SIZE] = {0};
    size_t stack_size = 0;

    for (size_t i = 0; i < instructions->times; i++) {
        stack_of_crates[i] = stack->crates[instructions->from][stack->crates_height[instructions->from] - 1];
    ...
    do {
        stack->crates[instructions->to][stack->crates_height[instructions->to]] = stack_of_crates[stack_size - 1];
